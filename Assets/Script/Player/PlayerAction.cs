...
public class PlayerAction : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        {
            if (verticalMove != Vector3.zero)  // 수직 방향키 입력이 있다면
            {
                ...
                transform.position += verticalMove * ladderSpeed * Time.deltaTime;    // 플레이어 이동
        ...
        {
            bool isMoving;
            if (horizontalMove != Vector3.zero) isMoving = true;
            else isMoving = false;
            ...
            rigid.velocity = new Vector2(horizontalMove.x * maxSpeed, rigid.velocity.y);

            if (horizontalMove == Vector3.right && !facingRight)
                Flip();
